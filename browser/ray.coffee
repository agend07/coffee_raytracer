class Canvas
    ...
    plot: (x, y, colour=[0, 0, 0]) ->
        y = @height - y
        [r, g, b] = colour
        index = (x + y * @width) * 4
        @canvasData.data[index + 0] = @putInRange r
        @canvasData.data[index + 1] = @putInRange g
        @canvasData.data[index + 2] = @putInRange b
        @canvasData.data[index + 3] = 255 # alpha

    updateCanvas: ->
        @ctx.putImageData @canvasData, 0, 0

    paintBackground: (color="#ffeeaa") ->
        @ctx.fillStyle = color
